module Cms
  class AdvertisingEventsController < BaseController
    def index
      @advertising_events = AdvertisingEvent.order('updated_at DESC').page(params[:page])
    end

    def new
      @page_title = t('cms.advertising_events.title')
      @advertising_event = AdvertisingEvent.new
    end

    def create
      if advertising_event = AdvertisingEvent.create(advertising_event_params)
        redirect_to cms_advertising_event_path(advertising_event)
      end
    end
    ...
    def destroy
      if advertising_event = AdvertisingEvent.find_by_id(params[:id])
        ...
        redirect_to cms_advertising_events_path
      end
    end
    ...
    def update
      if advertising_event = AdvertisingEvent.friendly.find(params[:id])
        ...
        redirect_to cms_advertising_event_path(advertising_event.id)
      end
    end
